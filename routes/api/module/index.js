...
router.get('/list', async (req, res, next) => {
  let conn = await pool.getConnection(async _conn => _conn)
  let [rows] = await conn.query('SELECT * FROM fiveworks_aurora_db.`ksa_moduleList` where online_show ="T"')
  conn.release()
  res.status(200).send(rows)
})
router.get('/board/list', async (req, res, next) => {
  let conn = await pool.getConnection(async _conn => _conn)
  let [rows] = await conn.query('SELECT * FROM fiveworks_aurora_db.`ksa_moduleList` where offline_show = "T"')
  conn.release()
  res.status(200).send(rows)
})
